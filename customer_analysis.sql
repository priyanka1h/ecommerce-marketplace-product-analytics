...
/* identifying demand market - highest no of order for each state*/
...
/*states contributing to the highest revenue*/
select c.customer_state,
sum(o_item.price) as high_price
from customers c
join orders o
...
/*depicting which customers place repeat orders*/
   select 
   c.customer_unique_id,
...
sum(o_item.price) as order_value
from customers c
join orders o
on c.customer_id = o.customer_id
join order_items as o_item
